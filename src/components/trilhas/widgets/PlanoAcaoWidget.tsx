...
export default function PlanoAcaoWidget({ trilhaSlug, etapa, resposta, contextoTrilhaRespostas }: WidgetProps) {
  ...
  const [acoes, setAcoes] = useState<Acao[]>(() => {
    if (done && resposta?.resposta) {
      return (resposta.resposta as { acoes: Acao[] }).acoes;
    }
    // eslint-disable-next-line react-hooks/purity
    const defaultPrazo = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    return topGaps.map((g) => ({ competencia_id: g.competencia.id, descricao: '', prazo: defaultPrazo }));
  });
