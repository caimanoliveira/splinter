...
const DEFAULT_PRAZO = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
...
export default function PlanoAcaoWidget({ trilhaSlug, etapa, resposta, contextoTrilhaRespostas }: WidgetProps) {
  ...
  const [acoes, setAcoes] = useState<Acao[]>(() => {
    ...
    return topGaps.map((g) => ({ competencia_id: g.competencia.id, descricao: '', prazo: DEFAULT_PRAZO }));
  });
